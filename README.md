...
[![Version](https://img.shields.io/badge/Version-2.5.0-green.svg)](https://github.com/villyoshioka/mati/releases)
...
3. **SEO対策** — Googleなどの検索エンジンに「このサイトの正当な持ち主です」と証明する設定（所有権確認）を、難しいコードに触れずに行えたり検索にヒットしやすくなります。検索結果に表示されるページの説明文（メタディスクリプション）も自動で設定され、トップページの説明文は自由に書くこともできます。記事のタイトルや公開日、サイトの運営者などの情報は、検索エンジンが読み取りやすい形（構造化データ）で伝えます。運営者の SNS アカウントなどの関連URLもあわせて伝えられます。
