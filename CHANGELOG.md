...
- **運営者の関連URL**: JSON-LD構造化データの出力を有効にしているとき、X や YouTube などのアカウントのURLを、運営者の関連URLとして最大5個まで追加できるようになりました。追加したURLは、SNS本人認証で設定したアカウントとあわせて運営者の情報として出力されます。
...
- **トップページの説明文**: メタディスクリプションの出力を有効にしているとき、トップページの説明文を自由に入力できるようになりました。入力した説明文は構造化データのサイト説明にも使われます。空欄の場合は、これまで通りサイトのキャッチフレーズを使います。
- **運営者の設定**: JSON-LD構造化データの出力を有効にしているとき、サイトの運営者を「団体」「個人」から選び、運営者名を指定できるようになりました。運営者名が空欄の場合はサイト名を使います。

### 改善

- **JSON-LD構造化データ**: 検索エンジンに伝える情報を強化しました。記事のタイトル・公開日・更新日を出力し、パンくずリストにはカテゴリーを含めるようにしました。SNS本人認証で設定したアカウントも運営者の情報として出力し、サイト・運営者・記事の情報を相互に関連付けています。

### バグ修正

- サイト名や記事タイトルに含まれる記号が、構造化データ内で正しく扱われない問題を修正しました。
- 設定に引用符を含む値を保存すると、不要な記号が付加されることがある問題を修正しました。
...
- **メディアファイル用ヘッダーの案内**: 「メディアファイル用ヘッダーの設定」の案内に「非表示にする」を追加しました。サーバーへの設定が済んだら案内を隠しておけます。設定の変更などで貼り付ける内容が変わると、案内は再び表示されます。
- **ヘッダーの適用範囲**: X-Robots-Tag と TDMRep の適用範囲を、2つのスイッチから1つの選択肢にまとめました。「ページとメディアファイルの両方」「X-Robots-Tag のみメディアファイルに限定」「両方ともメディアファイルに限定」から選べます。これまでの設定はそのまま引き継がれます。
...
- 連携プラグイン（CarryPod）の推奨バージョンを 3.3.1 に更新しました。

### 機能追加

- **TDMRep対応**: 「AI学習防止メタタグを追加」を有効にすると、TDM Reservation Protocol（TDMRep）によるテキスト・データマイニングの拒否表明もあわせて行うようになりました。詳細なライセンス条件や連絡先を記載したページのURLも指定できます。
- **X-Robots-Tagの適用範囲**: X-Robots-Tagをページとメディアファイル（アップロードフォルダ内のファイル）の両方に付けるか、メディアファイルのみに付けるかを選べるようになりました。TDMRepも同じ範囲に合わせるかどうかを選べます。メディアファイルにヘッダーを付けるためのサーバー設定は、設定画面に表示されます。
...
- **メタディスクリプション**: SEO設定に「メタディスクリプションを出力」を追加しました。記事の抜粋・本文やサイトのキャッチフレーズからページごとの説明文を自動で設定します。テーマや他のプラグインが出力する説明文よりも優先されます。
...
- コンテンツ保護機能を強化しました。保護の検出処理を全面的に見直し、これまで検出できていなかったケースにも対応しています。
...
- スマートフォン・タブレット等のモバイル端末で、画像ドラッグ禁止を有効にしていても画像の長押しメニューを制限できていなかった問題を修正しました。
...
- **テキスト選択禁止のカテゴリー指定**: テキスト選択禁止をカテゴリーごとに制御できるようになりました。テキスト選択禁止がONのときはチェックしたカテゴリーを制限から除外し、OFFのときはチェックしたカテゴリーのみ制限します。
...
- **バージョン通知**: 新しいバージョンが公開された際に、Mati設定ページにお知らせを表示するようになりました。GitHubのリリースページへのリンクが表示されます。
...
- スマートフォン・タブレット等のモバイル端末で、コンテンツ保護機能が意図せず発動してしまうことがある問題を修正しました。
...
1. [Releases](https://github.com/villyoshioka/Mati/releases) から v2.0.0 の ZIP ファイルをダウンロード
2. WordPress管理画面 → プラグイン → Mati を無効化（推奨）  
...
- **Google Analytics 測定ID対応**: SEO設定にGoogle Analytics の測定IDの入力欄を追加しました。

### 改善

- **管理メニューアイコンをオリジナルに変更**: Mati専用のSVGアイコンに変更しました。
...
- **開発者ツール抑制の強化**: 抑制をより強力にしました。
- **画像ドラッグ防止の強化**: より確実に防止するようになりました。
- **テキスト選択禁止の設定を移動**: 「コンテンツ保護設定」から「その他の設定」に移動しました。
- **ツールチップのテキストを簡潔化**: 設定画面の説明文を簡潔に書き直しました。
- **プレースホルダーテキストの改善**: SNS本人認証の入力欄の説明文をより分かりやすい表現に変更しました。
- **CarryPod実行中の操作無効化**: CarryPodの静的化実行中はMatiの設定変更操作を自動的に無効化するようになりました。

### バグ修正

- macOSの一部のキーボードショートカットがブロックされていなかった問題を修正しました。
