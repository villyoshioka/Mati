...
- **noarchiveタグの追加**: robots メタタグで検索エンジンのキャッシュを防止する `noarchive` ディレクティブを追加しました。
- **JSON-LD構造化データ**: WebSite、Organization、BreadcrumbList の JSON-LD 構造化データを出力する機能を追加しました。SEO向上に効果的です。

### 改善

- **デベロッパーツールブロックの強化**: キーボードショートカット無効化機能を改善し、JavaScript難読化を実装しました。
  - F12キー、Ctrl+Shift+I/J/C、Cmd+Option+I/J/C、Ctrl+U/Cmd+U のブロックを実装
  - ローカル環境（localhost、.local、127.0.0.1）では自動的に無効化
  - モバイル端末では自動的に無効化
  - JavaScript コードを難読化し、保護機能の解析を困難にしました
...
- 保護機能のJavaScriptコードを難読化し、解析を困難にしました。
